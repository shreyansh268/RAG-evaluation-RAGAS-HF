...
class ExampleRAG:
    ...
    def generate_response(self, query: str, top_k: int = 3) -> str:
        ...
        is_anthropic = hasattr(self.llm_client, "messages") and not hasattr(self.llm_client, "chat")
        if self.model:
            model = self.model
        elif is_anthropic:
...
def default_rag_client(llm_client, logdir: str = "logs", model: Optional[str] = None) -> ExampleRAG:
    """
    ...
    Args:
        ...
        model: Model name to use (overrides default detection)
    Returns:
        ExampleRAG instance
    """
    retriever = SimpleKeywordRetriever()
    client = ExampleRAG(llm_client=llm_client, retriever=retriever, logdir=logdir, model=model)
